...
const ounass = (id: string): BrandRead => ({
  ...
  bestsellers: s(
    ...
    [
      ev("ou-sort", "page", "category default sort", "“Most popular” is the default sort on all four sampled categories; position 1 to 4 recorded over 4 weeks."),
  ...
  socials: s(
    [
      { platform: "Instagram", handle: "@ounass", followers: 1_240_000, note: "Posts 5 to 6 times a week; Reels are 70% of the last 30 posts." },
    ...
    [
      ...
      ev("ou-ig", "social", "@ounass on Instagram", "1.24M followers, 5 to 6 posts a week, 21 of the last 30 are Reels.", "checked just now"),
  ...
  markets: s(
    [
      ...
      { code: "KW", name: "Kuwait", share: 9, note: "Delivery in 2 to 3 days. No dedicated landing page." },
      { code: "QA", name: "Qatar", share: 7, note: "Delivery in 2 to 3 days." },
  ...
  seasonality: s(
    [
      { label: "Ramadan and Eid", window: "Feb to Mar", lift: "Highest of the year", note: "A dedicated /ramadan landing page appears each year and is linked from the header." },
      { label: "Summer resort", window: "May to Jun", lift: "Second peak", note: "The Resort Edit ran in each of the last three years." },
    ...
    [
      ...
      ev("ou-cal", "platform", "HeyMoon GCC calendar", "Ramadan begins 18 Feb 2026. Gulf luxury demand peaks in the ten days before Eid."),
    ]
  ),
  eligibility: {
    state: "ok",
    line: s(
      "You qualify for a guarantee. About 280,000 visitors a month, well above the minimum.",
      "Guaranteed sales are attributed through one tracking link per creator. Below roughly 5,000 monthly visitors that attribution gets too noisy to guarantee, so the floor exists to protect the guarantee, not to gate the product.",
      [
        ev("ou-traffic", "platform", "traffic estimate", "280,000 monthly unique visitors, from HeyMoon's panel and public rank data.", "estimated this month"),
...
const luna = (id: string): BrandRead => ({
  ...
  markets: s(
    [
      ...
      { code: "SA", name: "Saudi Arabia", share: 24, note: "Ships in 3 to 5 days; no SAR pricing." },
      { code: "KW", name: "Kuwait", share: 15, note: "Ships in 3 to 5 days." },
    ],
  ...
  seasonality: s(
    [
      { label: "Ramadan gifting", window: "Feb to Mar", lift: "Strongest", note: "The gift sets exist for this window; they appeared last year in early February." },
      { label: "Summer barrier care", window: "Jun to Aug", lift: "Steady", note: "Journal posts pivot to heat and SPF." },
    ],
  ...
  eligibility: {
    state: "ok",
    line: s(
      "You qualify for a guarantee. About 41,000 visitors a month, well above the minimum.",
...
const fresh = (id: string): BrandRead => ({
  ...
  category: s(
    ...
    "Perishables with a delivery-window picker. That is grocery, which behaves differently from every other category on HeyMoon: high repeat, low basket, no seasonality to speak of.",
    [
  ...
  seasonality: s(
    [{ label: "Ramadan", window: "Feb to Mar", lift: "Basket size up, order count flat", note: "Iftar bundles appeared last year; the rest of the year is level." }],
    "Grocery is the flattest category on the platform. One window moves, and it moves basket size rather than order count.",
    [ev("fg-flat", "benchmark", "grocery on HeyMoon", "Across grocery brands, month-to-month order variance is under 8%.")]
  ),
  eligibility: {
    state: "short",
    line: s(
      "Not eligible yet. 3,200 monthly visitors against the 5,000 a guarantee needs",
      "Guaranteed sales are attributed through one tracking link per creator. Below roughly 5,000 monthly visitors there are too few conversions per link to tell a creator's effect from noise, and HeyMoon pays you the difference when a guarantee misses, so a guarantee on thin traffic is a coin flip for both of us.",
      [
        ev("fg-traffic", "platform", "traffic estimate", "3,200 monthly unique visitors, from HeyMoon's panel and public rank data.", "estimated this month"),
    ...
    learned: [
      ...
      "Grocery is the flattest category HeyMoon measures, so a campaign here would be sized on repeat rate rather than on a seasonal peak.",
    ...
    wayBack: [
      { label: "Nothing to reapply for", detail: "HeyMoon re-checks automatically. The moment freshgrocer.ae crosses 5,000 monthly visitors, guaranteed campaigns unlock on their own and you get an email." },
      ...
      { label: "Your store details are saved", detail: "When you come back, the campaign starts from everything above rather than from an empty form." },
      { label: "If this number looks wrong", detail: "Most analytics dashboards show sessions, which run higher than unique visitors. Ask for a manual re-check and a person will verify by hand within two working days." },
    ],
  },
});
...
export function fixtureFor(url: string): (id: string) => BrandRead {
  ...
  return (id: string) => {
    ...
    return {
      ...
      url: key,
      identity: {
        name: s(pretty, `Taken from the domain, because ${key} is a store HeyMoon has not read before.`, [
          ev("gx-dom", "page", key, "No cached read. Everything below is a first pass and should be corrected freely."),
        ]),
        logo: undefined,
        tagline: s("A store HeyMoon is reading for the first time", "There is no cached profile for this domain.", [
