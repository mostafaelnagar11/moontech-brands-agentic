...
/* The seven agents MoonTech actually runs, in the order they run in.
   `job` is one line now rather than one sentence — a dashboard row is
   read at a glance or not at all. `needsYou` is filled in only for the
   two that cannot finish alone: MoonLive AI, because it publishes, and
   MoonScore AI, because the next phase costs money. Both match the
   locked rules below rather than restating them. */
const PIPELINE: { agent: string; stage: string; job: string; needsYou?: string }[] = [
  ...
  },
  {
    ...
    needsYou: "Starting a phase — any new money — is yours to confirm.",
  },
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ── The pipeline ─────────────────────────────────────────────
          ...
          the lock on the two that cannot finish alone. Below sm the
          job wraps under the name rather than being squeezed. */}
      <Section
